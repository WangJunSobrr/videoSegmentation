...
void VideoSnapCut::generateContourWithRect(Mat& tmp)
{
	...
	Mat con1;
	con.copyTo(con1);
	for(p1.x = 0; p1.x < con.cols; p1.x++)
	{
		for(p1.y = 0; p1.y < con.rows; p1.y++)
		{
			if(con.at<uchar>(p1) != 0)
			{
				con1.at<uchar>(p1) = 255;
			}		
		}
	}
	imshow("找轮廓之前的mask", con1);
	Mat element = getStructuringElement(MORPH_RECT, Size(2, 2));
	erode(con, con, element);
	dilate(con, con, element); 
